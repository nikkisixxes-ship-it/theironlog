...
async function collBuildCustomExerciseDeletionDisclosure(exId, attempt) {
  ...
  for (const cid of referencedCollectionIds) {
    ...
    collections.push({ collectionId: cid, collectionName: live.name, branch: check.branch });
  }
...
async function collRunCustomExerciseDeletion(exId, disclosure, callbacks) {
  ...
  try {
    result = await fsDb.runTransaction(async (tx) => {
      ...
      const memberSnap = await tx.get(memberRef); // 1. membership doc, fresh
      ...
      // Validate the COMPLETE membership record -- both arrays and the role --
      // against exactly what was disclosed, not just the membership array.
