// ==================== EXERCISE COLLECTIONS (Stage 2) ====================
// Stage 2 scope: convert the approved Stage 1B interface from temporary local
// data into durable, concurrency-safe Firestore data. Collection identity,
// membership, and consolidation *configuration* are now persisted. Consolidation
// remains inert data -- it has no effect on the live Exercise List, no exclusivity
// enforcement, no transfers, no roles in the reverse index. That is all deferred
// to Stage 3A/3B.
//
// Ownership (unchanged from Stage 1A/1B):
...
//     the Collection draft model and its Stage 2 persistence, the Library route
//     model, and the unsaved-change predicate/discard helper that app-core.js's
//     showPage() guard calls into.
...
// never rendered, and every write to it uses a complete tx.set() (never tx.update(),
...
// whether it should be a member of `collectionId` after this operation. Always a
// complete tx.set() with a freshly computed document -- never tx.update(), since
// the document may not have existed before this call. consolidationRole is always
// null in Stage 2 (Section 6) -- no role is ever read, preserved, or written.
function collComputeMembershipWrite(entry, collectionId, isMember) {
...
function collHandleCollectionsSnapshotUpdate() {
  ...
  // TRAIN/PLAN: the shared Exercise Browser modal, only while it is actually
  // showing Collections content (list or a focused member view).
  if (browserModalOpen && libBrowseTarget === 'collections') {
    if (libCollectionDrill && !collFindCollection(libCollectionDrill.collectionId)) {
...
function collRenderEditor() {
  ...
  const deleteBtnHtml = !isNew ? `
    ...
    <button class="btn btn-danger" style="width:100%" onclick="collConfirmDeleteCollection()" ${collPendingOperation ? 'disabled' : ''}>Delete Collection</button>
  ` : '';
  ...
  page.innerHTML = `
    ...
    <div style="padding:16px">
      <div class="form-group">
        ...
        <input type="text" id="coll-name-input" value="${escapeHtml(collectionDraft.name)}" placeholder="e.g. Bench Press Variants" oninput="collOnNameInput(this.value)">
      </div>
...
async function collRunCreateTransactionAttempt(payload) {
  return fsDb.runTransaction(async (tx) => {
    ...
    const writes = payload.memberIds.map(exId => collComputeMembershipWrite(membershipEntries.get(exId), payload.id, true));
    tx.set(ref, payload);
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (err && err.collReason === 'uidCollision') {
        lastErr = err;
        candidateId = uid(); // regenerate and retry; draft itself is never touched
        continue;
      }
...
async function collSaveNewCollection() {
  ...
  const result = await collCreateCollectionTransaction(collectionDraft);
...
async function collSaveExistingCollection() {
  ...
  try {
    result = await fsDb.runTransaction(async (tx) => {
      ...
      const writes = affectedIds.map(exId => collComputeMembershipWrite(membershipEntries.get(exId), finalPayload.id, memberSet.has(exId)));
  ...
  try {
    result = await fsDb.runTransaction(async (tx) => {
      ...
      if (memberIds.length) {
        ...
        // -- read phase complete; no writes issued above this line --
        writes = memberIds.map(exId => collComputeMembershipWrite(membershipEntries.get(exId), id, false));
      }
      tx.delete(ref);
      collApplyMembershipWrites(tx, writes);
