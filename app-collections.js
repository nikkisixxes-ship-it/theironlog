...
//
// Ambiguous data (the same Exercise ID claimed -- as Display or as a consolidated
// member -- by more than one valid-shaped group, across any Collections) is never
// resolved by picking a winner: every exercise ID with more than one claim is
// excluded from the projection entirely, so it always falls back to an ordinary,
// individually-visible row. If the contested ID was itself a Display Exercise,
// that whole group becomes unusable; if it was one of several consolidated
// members, only that member drops out (the Display Exercise still shows the
// remaining valid members, with an accordingly reduced Variants count).
function collGetConsolidationProjection() {
  const claims = new Map(); // exerciseId -> claim count (across all groups/roles)
  function noteClaim(exId) { claims.set(exId, (claims.get(exId) || 0) + 1); }
  ...
  for (const c of collections) {
    ...
    noteClaim(displayId);
    validMemberIds.forEach(noteClaim);
  }

  const contested = new Set();
  claims.forEach((count, exId) => { if (count > 1) contested.add(exId); });
