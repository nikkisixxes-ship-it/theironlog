...
let libShowAllIndividually = false; // Exercises-mode only; suspends consolidation collapsing on the default list (inert until Stage 3A)
...
// through this fixed, three-method contract. If Collections content is requested
...
function openExBrowser(purpose, onSelectCallback, context) {
  ...
  // Every fresh open starts un-drilled, with consolidation collapsing suspended --
  // both are always session-local to one open/purpose, never resumed across opens.
...
// -- Consolidation rollout gate (Stage 1B: pure check only, not wired into rendering) --
// True only in the Exercise list's true default state -- no active category/type
// filter, no search, alpha sort, and Show All Individually off. This is the single
// condition under which Stage 3A will later be allowed to collapse consolidated
// members under their Display Exercise. In Stage 1B this helper exists and is
// independently testable, but renderExerciseList() never consults it -- the real
// Exercise List has no visible consolidation behavior yet.
...
// Suspends consolidation collapsing on the default Exercise List (inert until Stage 3A).
