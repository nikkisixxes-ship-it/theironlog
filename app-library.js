...
// Entry points:
//   openExBrowser('browse', null)        -- Library tab
//   openExBrowser('select', fn(exId))    -- Logger / Plan / Change

// -- Browser State --
let libMode = 'browse';        // 'browse' | 'select' -- set by parent, never mutated by core
let libOnSelect = null;        // fn(exId) | null -- injected by parent

// Filter state -- persists within session except search and display (always reset on open)
let libState = {
  ...
  displaySystem:  true,
  displayCustom:  true,
  sort: 'alpha',               // 'alpha' | 'recent' -- persists across opens
  // search is NOT stored here; lives only in DOM, always reset on open
};

// -- Single Entry Point --
function openExBrowser(mode, onSelectCallback) {
  libMode     = mode || 'browse';
  libOnSelect = onSelectCallback || null;

  // Search always resets on open
  const searchEl = document.getElementById('lib-search');
  if (searchEl) searchEl.value = '';

  // Display filters reset on every open (not persistent)
  libState.displaySystem = true;
  libState.displayCustom = true;

  // Note: activeCategories, movementStructure, executionStyle persist within session
  // Note: sort persists as user preference across opens
  ...
  if (!exs.length) {
    ...
    listEl.innerHTML = `
      <div class="lib-empty">
        ...
        ${hasSearch ? `<button class="btn btn-ghost btn-sm" style="margin-top:8px" onclick="document.getElementById('lib-search').value='';renderExerciseList()">Clear Search</button>` : ''}
      </div>`;
    return;
  }
...
function openAddExerciseModal() {
  openExBrowser('select', exId => addExToWorkout(exId));
}

function openChangeExModal(exIdx) {
  changeExTargetIdx = exIdx;
  openExBrowser('select', exId => confirmChangeExercise(exId));
}
...
function openExBrowserSelect(callback) { openExBrowser('select', callback); }
