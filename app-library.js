...
//   openExBrowser('select', fn(exId), 'plan', { multiSelect: true }) -- Plan Quick Add (stays open, Done to close)
//
...
let libMultiSelect = false;    // when true, selecting an exercise stays in the browser instead of closing it
...
function openExBrowser(mode, onSelectCallback, context, options) {
  libMode     = mode || 'browse';
  libOnSelect = onSelectCallback || null;
  libMultiSelect = !!(options && options.multiSelect);
  ...
  return `
    <div class="lib-shell${isSelect ? ' lib-shell--modal' : ' lib-shell--page'}">
      ...
      <div class="lib-footer">
        ${libMultiSelect ? `<button class="btn btn-primary" onclick="closeModal('modal-ex-browser')">Done</button>` : ''}
...
function libAcceptRandom(exId) {
  ...
  if (libMultiSelect) renderExerciseList();
}
