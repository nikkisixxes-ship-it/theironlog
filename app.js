...
function onSetChecked(exIdx, si, checked) {
  ...
  // Update wrap class for visual dimming
...
function buildExerciseBlock(ex, idx) {
  ...
  // Meta badges (Last + TM) — visual only, no click action
  let metaBadgesHtml = '';
  if (prev) {
    metaBadgesHtml += `<span class="ex-meta-badge">Last: ${prev.replace(appDb.unit,'').replace(appDb.unit.toLowerCase(),'').trim()}</span>`;
  }
  if (currentWorkout.programId) {
    ...
    if (prog) {
      ...
      if (tmData && tmData.value) {
        metaBadgesHtml += `<span class="ex-meta-badge">TM: ${tmData.value}</span>`;
      } else if (tmData && tmData.repGoalBase != null) {
        metaBadgesHtml += `<span class="ex-meta-badge">Base: ${tmData.repGoalBase}</span>`;
      }
    }
  }
  ...
  // Notes section — visual only Hide button
  const notesHtml = note ? `
    <div>
      <div class="ex-notes-label">Setup Notes <span class="ex-notes-hide-btn">Hide</span></div>
      <div class="ex-notes-content">${escapeHtml(note)}</div>
    </div>` : '';
  ...
  return `<div class="exercise-item" id="ex-block-${idx}">
    ...
    ${metaBadgesHtml ? `<div class="exercise-meta-row">${metaBadgesHtml}</div>` : ''}
    ${tmControls}
    ${notesHtml ? `<div style="margin-top:24px">${notesHtml}</div>` : ''}
    <div style="margin-top:24px">
      ...
      <div class="ex-action-row">
        <button class="btn btn-ghost" style="background:#373737;border-color:#5C5C5C" onclick="addSet(${idx})">+ Set</button>
        <button class="btn btn-ghost" onclick="addPRSet(${idx})" style="background:#373737;color:var(--accent2);border-color:rgba(255,170,0,0.4)">+ PR Set</button>
      </div>
    </div>
  </div>`;
}
...
function buildSetsHTML(ex, exIdx, isTime, rpe) {
  ...
  return ex.sets.map((s, si) => {
    ...
    // PR badge
    const prVal = s.isNew1RM && s.weight && s.reps ? `PR: ${Math.round(calc1RM(+s.weight, +s.reps))}` : (s.isPR ? `PR: ${s.weight}` : '');
    const prBadge = prVal ? `<div class="set-pr-badge">${prVal}</div>` : '';
