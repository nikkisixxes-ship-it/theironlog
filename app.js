...
function getActivePrograms() {
  // Active = has at least one completed session AND is not complete
  return appDb.programs.filter(p => {
    if (!p.completedSessionIndices || p.completedSessionIndices.length === 0) return false;
    const totalSessions = p.microcycles.reduce((a, mc) => a + mc.sessions.length, 0);
    return p.completedSessionIndices.length < totalSessions;
  });
}
...
function renderHomeCarousel() {
  ...
  carousel.innerHTML = homeCarouselProgs.map((prog, i) => {
    ...
    const sessionLabel = `Week ${next.microcycleIdx + 1} \u2022 ${sessName}`;
    ...
    return `<div class="prog-carousel-card" onclick="startWorkoutFromCard('${prog.id}')">
      <div style="display:flex;align-items:flex-start;justify-content:space-between">
        <div style="flex:1;min-width:0">
          <div class="prog-card-session">${sessionLabel}</div>
          <div class="prog-card-name">${prog.name}</div>
        </div>
        <button class="prog-card-menu-btn" onclick="openProgCardMenu(event,'${prog.id}')" title="Program options">•••</button>
      </div>
...
function openProgramFromCard(progId) {
  ...
  setTimeout(() => {
    const el = document.getElementById(`prog-card-${progId}`);
    if (el) {
...
function endProgramFromCard(progId) {
  ...
  confirm2(
    'End Program?',
    'This program will be removed from Active Programs and reset to its starting state. Workout history will remain available in History.',
    () => {
...
function finishWorkout() {
  ...
  if (currentWorkout._isEdit) {
    // Overwrite existing workout record — preserve original duration
    const originalDuration = currentWorkout.duration;
    delete currentWorkout._isEdit;
    currentWorkout.duration = originalDuration;
    ...
    const timerEl = document.getElementById('log-timer-display');
    if (timerEl) { timerEl.style.opacity = ''; timerEl.style.pointerEvents = ''; }
    stopWorkoutTimer(); currentWorkout = null;
    showToast('Workout updated!', 'success'); showPage('page-home');
    return;
  }
  ...
  if (currentWorkout.programId) {
    ...
    if (prog) {
      ...
      // Auto-end if all sessions complete
      const totalSessions = prog.microcycles.reduce((a, mc) => a + mc.sessions.length, 0);
      if (prog.completedSessionIndices.length >= totalSessions) {
        prog.completedSessionIndices = [];
        prog.trainingMaxes = {};
...
function getWorkoutHighlights(w) {
  // For each exercise in workout order, find its best set.
  // Weighted: best e1RM. Bodyweight: most reps. Time: longest duration.
  // Skip exercises with no usable data. Return first 2.
  const highlights = [];
  for (const e of w.exercises) {
    if (highlights.length >= 2) break;
    const exData = getExercise(e.exerciseId);
    const name = exData ? exData.name : e.exerciseId;
    const isTime = exData && (exData.tracking === 'time' || exData.tracking === 'weight_time');
    const isBW = exData && exData.tracking === 'bodyweight_reps';
    const isWeighted = !isTime && !isBW;

    let bestSet = null;
    let bestVal = -1;

    for (const s of e.sets) {
      if (isWeighted && s.weight && +s.weight > 0 && s.reps && +s.reps > 0) {
        const v = calc1RM(+s.weight, +s.reps);
        if (v > bestVal) { bestVal = v; bestSet = s; }
      } else if (isBW && s.reps && +s.reps > 0) {
        if (+s.reps > bestVal) { bestVal = +s.reps; bestSet = s; }
      } else if (isTime && s.time && +s.time > 0) {
        if (+s.time > bestVal) { bestVal = +s.time; bestSet = s; }
      }
    }
...
function renderHistoryList() {
  ...
  if (!wks.length) {
    list.innerHTML = '<div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="12 8 12 12 14 14"/><path d="M3.05 11a9 9 0 1 0 .5-4"/></svg><h3>No Workouts Found</h3><p>Start logging to see history here</p></div>';
    return;
  }
  const visible = wks.slice(0, historyShown);
  const showMoreBtn = wks.length > historyShown
    ? `<div style="padding:16px;text-align:center"><button class="btn btn-secondary" onclick="showMoreHistory()">Show More</button></div>`
    : '';
...
function renderRecentSearches() {
  ...
  el.innerHTML = `<div style="font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--text3);margin-bottom:8px">Recent Searches</div>`
    + searches.map(s => {
      ...
      return `<div onclick="applyRecentSearch(this.dataset.term)" data-term="${safe}" style="padding:10px 12px;border-bottom:1px solid var(--border);cursor:pointer;display:flex;align-items:center;justify-content:space-between;font-family:var(--font-display);font-size:15px;font-weight:700;text-transform:uppercase;color:var(--text2)" onmouseenter="this.style.background='var(--surface2)'" onmouseleave="this.style.background=''">
        <span>${safe}</span>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--text3)"><polyline points="9 18 15 12 9 6"/></svg>
      </div>`;
    }).join('');
}

function applyRecentSearch(term) {
  document.getElementById('history-search-input').value = term;
  runHistorySearch();
}
...
function runHistorySearch() {
  ...
  const resultsEl = document.getElementById('history-search-results');

  if (!q) { renderRecentSearches(); resultsEl.innerHTML = ''; return; }
  renderRecentSearches();
  ...
  const matches = appDb.workouts.filter(w => {
    ...
    const dateStr = d.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'}).toLowerCase();
    ...
    if (monthYear.includes(q)) return true;
    if (w.exercises.some(e => { const ex = getExercise(e.exerciseId); return ex && ex.name.toLowerCase().includes(q); })) return true;
    return false;
  });

  if (!matches.length) {
    resultsEl.innerHTML = '<div style="padding:20px;text-align:center;color:var(--text3);font-size:14px">No results found</div>';
    return;
  }
  ...
  resultsEl.innerHTML = `<div style="font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--text3);margin-bottom:8px">${matches.length} Result${matches.length !== 1 ? 's' : ''}</div>`
    + matches.map(w => {
        ...
        return `<div onclick="goToSearchResult('${w.id}')" style="padding:12px;border-bottom:1px solid var(--border);cursor:pointer" onmouseenter="this.style.background='var(--surface2)'" onmouseleave="this.style.background=''">
          <div style="font-size:11px;color:var(--text3);margin-bottom:3px">${d}</div>
          <div style="font-family:var(--font-display);font-size:16px;font-weight:800;text-transform:uppercase;color:var(--text)">${w.name}</div>
          <div style="font-size:12px;color:var(--text3);margin-top:2px">${w.exercises.length} exercises · ${sets} sets</div>
        </div>`;
      }).join('');
}
...
function goToSearchResult(wid) {
  const raw = document.getElementById('history-search-input').value.trim();
  if (raw) saveRecentSearch(raw);
  closeModal('modal-search-history');
  const idx = appDb.workouts.findIndex(w => w.id === wid);
  if (idx >= historyShown) { historyShown = idx + historyPageSize; }
  // Navigate to history page without resetting historyShown
  document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
  document.getElementById('page-history').classList.add('active');
  document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active'));
  renderHistoryStats();
  renderHistoryList();
  setTimeout(() => {
    const card = document.getElementById(`hist-expanded-${wid}`);
    if (card) {
      if (card.style.display === 'none') toggleHistoryCard(wid);
      document.getElementById(`hist-collapsed-${wid}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  }, 150);
}
...
function openBrowseDateModal() {
  const yearEl = document.getElementById('browse-year');
  const years = [...new Set(appDb.workouts.map(w => new Date(w.date).getFullYear()))].sort((a,b) => b-a);
  if (!years.length) { showToast('No workouts yet', ''); return; }
  yearEl.innerHTML = years.map(y => `<option value="${y}">${y}</option>`).join('');
  // Default month to current
  document.getElementById('browse-month').value = new Date().getMonth();
  openModal('modal-browse-date');
}

function browseByDate() {
  const year = parseInt(document.getElementById('browse-year').value);
  const month = parseInt(document.getElementById('browse-month').value);
  closeModal('modal-browse-date');
  const match = appDb.workouts.find(w => {
    const d = new Date(w.date);
    return d.getFullYear() === year && d.getMonth() === month;
  });
  if (!match) { showToast('No workouts found for that month', ''); return; }
  const idx = appDb.workouts.indexOf(match);
  if (idx >= historyShown) { historyShown = idx + historyPageSize; }
  // Navigate manually to avoid renderHistory() resetting historyShown
  document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
  document.getElementById('page-history').classList.add('active');
  document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active'));
  renderHistoryStats();
  renderHistoryList();
  setTimeout(() => {
    const el = document.getElementById(`hist-collapsed-${match.id}`);
...
function workoutHistoryCard(w, compact=false) {
  ...
  // History screen — includes workout highlights
  const highlights = getWorkoutHighlights(w);
  const highlightsHtml = highlights.length
    ? `<div class="history-card-highlights" id="hist-highlights-${wid}">${highlights.map(h =>
        `<div class="history-highlight-row">
          <span class="history-highlight-name">${h.name}</span>
          <span class="history-highlight-val">${h.display}</span>
        </div>`
      ).join('')}</div>`
    : '';

  return `<div class="recent-card" onclick="toggleHistoryCard('${wid}')">
    <div class="recent-card-collapsed" id="hist-collapsed-${wid}">
      <div class="recent-card-main">
        <div class="recent-card-date">${d}</div>
        <div class="recent-card-name-row">
          <div class="recent-card-name">${w.name}</div>
...
function toggleHistoryCard(wid) {
  const expanded = document.getElementById(`hist-expanded-${wid}`);
  const highlights = document.getElementById(`hist-highlights-${wid}`);
  const chevron = document.querySelector(`#hist-collapsed-${wid} .recent-card-chevron`);
  if (!expanded) return;
  ...
  if (!isOpen) {
    expanded.innerHTML = buildRecentExpandedHtml(wid);
...
function toggleRecentCard(wid) {
  ...
  // Collapse all open cards first (accordion)
  document.querySelectorAll('.recent-card-expanded').forEach(el => {
    el.style.display = 'none';
  });
  document.querySelectorAll('.recent-card-chevron').forEach(el => {
    el.style.transform = '';
  });
...
function buildRecentExpandedHtml(wid) {
  const w = appDb.workouts.find(x => x.id === wid);
  if (!w) return '';
  ...
  const exercisesHtml = w.exercises.map((e, ei) => {
    ...
    const refVideos = getExVideos(e.exerciseId);
    const hasVideos = perfVideos.length > 0 || refVideos.length > 0;
    const videoIcon = hasVideos
      ? `<span class="recent-ex-video-btn" onclick="openExpandedVideosModal('${wid}',${ei});event.stopPropagation()">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--text2)" stroke-width="2.5"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
        </span>`
      : '';
    ...
    const setsHtml = e.sets.map(s => {
      ...
      const prBadge = s.isPR
        ? `<span class="recent-badge recent-badge-pr" style="font-size:10px;padding:1px 6px;display:inline-block;min-width:72px;text-align:center">PR: ${s.weight}</span>`
        : `<span style="display:inline-block;min-width:72px"></span>`;
      const e1rmBadge = s.isNew1RM && s.weight && s.reps
        ? `<span class="recent-badge recent-badge-1rm" style="font-size:10px;padding:1px 6px;display:inline-block;min-width:72px;text-align:center">E1RM: ${Math.round(calc1RM(+s.weight, +s.reps))}</span>`
        : (s.isPR ? `<span style="display:inline-block;min-width:72px"></span>` : '');

      return `<div class="recent-set-row">
        <span class="recent-set-weight">${weightStr}</span>
        <span class="recent-set-x">${xStr}</span>
        <span class="recent-set-reps">${repsStr}</span>
        <span class="recent-set-badges">${prBadge}${e1rmBadge}</span>
      </div>`;
    }).join('');

    return `<div class="recent-ex-block">
      <div class="recent-ex-name-row">
        <span class="recent-ex-num">${ei + 1}</span>
        <span class="recent-ex-name">${name}</span>
        ${videoIcon}
      </div>
      <div class="recent-ex-sets">${setsHtml}</div>
    </div>`;
  }).join('');

  return `<div class="recent-expanded-inner">
    <div class="recent-ex-list">${exercisesHtml}</div>
    <div class="recent-expanded-actions">
      ...
      <button class="btn btn-secondary" style="flex:1;justify-content:center" onclick="openCopyModal('${wid}');event.stopPropagation()">Export</button>
    </div>
  </div>`;
}
...
function openExpandedVideosModal(wid, exIdx) {
  ...
  const refHtml = refVideos.length
    ? `<div style="margin-bottom:12px"><div class="video-section-label">Reference Videos</div>${refVideos.map(v => `<div class="video-link-row"><a class="video-link" href="${v}" target="_blank">${v}</a></div>`).join('')}</div>`
    : '';
  const perfHtml = perfVideos.length
    ? `<div><div class="video-section-label">This Session</div>${perfVideos.map(v => `<div class="video-link-row"><a class="video-link" href="${v}" target="_blank">${v}</a></div>`).join('')}</div>`
    : '';
  body.innerHTML = refHtml + perfHtml || '<div style="color:var(--text3);font-size:14px">No videos found.</div>';
...
function editHistoricalWorkout(wid) {
  ...
  currentWorkout._isEdit = true;
  const nameDisplay = document.getElementById('log-session-name-display');
  if (nameDisplay) nameDisplay.textContent = (w.name || 'WORKOUT').toUpperCase();
  // Show original duration locked — don't start timer
  const timerEl = document.getElementById('log-timer-display');
  if (timerEl) { timerEl.textContent = w.duration ? fmtDuration(w.duration) : '0:00'; timerEl.style.opacity = '0.5'; timerEl.style.pointerEvents = 'none'; }
  document.getElementById('active-bar').classList.remove('visible');
  renderWorkoutExercises();
  showPage('page-log');
}
