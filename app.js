...
function buildExerciseBlock(ex, idx) {
  ...
  let metaBadgesHtml = '';
  if (prev) {
    metaBadgesHtml += `<span class="ex-meta-badge">Last: ${prev}</span>`;
  }
  if (currentWorkout.programId) {
    ...
    if (prog) {
      ...
      if (tmData && tmData.value) {
        metaBadgesHtml += `<span class="ex-meta-badge">TM: ${tmData.value}${appDb.unit}</span>`;
      } else if (tmData && tmData.repGoalBase != null) {
        metaBadgesHtml += `<span class="ex-meta-badge">Base: ${tmData.repGoalBase}${appDb.unit}</span>`;
      }
    }
  }
  ...
  return `<div class="exercise-item" id="ex-block-${idx}">
    ...
    <div style="margin-top:24px">
      ...
      <div class="ex-action-row">
        <button class="btn btn-ghost" onclick="addSet(${idx})">+ Set</button>
        <button class="btn btn-ghost" onclick="addPRSet(${idx})" style="color:var(--accent2);border-color:rgba(255,170,0,0.4)">+ PR Set</button>
      </div>
    </div>
  </div>`;
}
