...
function finishWorkout() {
  ...
  if (currentWorkout._isEdit) {
    // Overwrite existing workout record
    delete currentWorkout._isEdit;
    currentWorkout.duration = w ? w.duration : getElapsedSeconds();
...
function openSearchHistoryModal() {
  ...
  document.getElementById('history-search-recent').style.display = '';
...
function renderRecentSearches() {
  ...
  if (!searches.length) { el.innerHTML = ''; return; }
...
function applyRecentSearch(termOrEl) {
  const term = typeof termOrEl === 'string' ? termOrEl : termOrEl;
  document.getElementById('history-search-input').value = term;
  document.getElementById('history-search-recent').style.display = 'none';
  runHistorySearch();
}
...
function runHistorySearch() {
  ...
  const recentEl = document.getElementById('history-search-recent');

  if (!q) { recentEl.style.display = ''; resultsEl.innerHTML = ''; return; }
