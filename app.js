...
function buildExerciseBlock(ex, idx) {
      ...
      if (session) {
        ...
        if (sessEx?.sets) {
          ...
          if (hasPct || hasRG) {
            ...
            const tmDisplay = tmData && tmData.value ? `TM: ${tmData.value}${appDb.unit}` : tmData && tmData.repGoalBase != null ? `Base: ${tmData.repGoalBase}${appDb.unit}` : null;
            tmControls = `<div style="display:flex;align-items:center;gap:8px;margin-top:3px;flex-wrap:wrap">${tmDisplay ? `<span style="font-size:12px;color:var(--accent2);font-family:var(--font-display);font-weight:600">${tmDisplay}</span>` : '<span style="font-size:12px;color:var(--red)">No baseline set</span>'}<button class="btn btn-ghost btn-sm" style="font-size:11px;padding:4px 9px" onclick="openSingleTMPrompt(${idx})">${hasPct?'Set TM':'Set Baseline'}</button></div>`;
          }
        }
      }
    }
  ...
  return `<div class="exercise-item" id="ex-block-${idx}"><div class="exercise-item-header"><div style="flex:1;min-width:0"><div class="exercise-item-name">${name}${exrxHtml}</div>${prev ? `<div class="exercise-prev">Last: ${prev}</div>` : ''}${tmControls}</div><button class="ex-menu-btn" onclick="openExerciseMenu(event,${idx})">•••</button></div>${notesHtml}<div class="${hdrCls}" style="margin-top:4px"><div class="set-label">✓</div><div class="set-label">${col2Label}</div><div class="set-label">${col3Label}</div>${rpe ? '<div class="set-label">RPE</div>' : ''}</div><div id="sets-container-${idx}">${buildSetsHTML(ex, idx, isTime, rpe)}</div><div class="ex-action-row"><button class="btn btn-ghost btn-sm" onclick="addSet(${idx})">+ Set</button><button class="btn btn-ghost btn-sm" onclick="addPRSet(${idx})" style="color:var(--accent2);border-color:rgba(255,170,0,0.4)">+ PR Set</button></div></div>`;
}
function buildSetsHTML(ex, exIdx, isTime, rpe) {
  ...
  return ex.sets.map((s, si) => {
    ...
    return `<div class="set-row-wrap" id="set-wrap-${exIdx}-${si}"><div class="${rowCls}" id="set-row-${exIdx}-${si}"><input type="checkbox" class="set-checkbox" ${s.completed?'checked':''} onchange="onSetChecked(${exIdx},${si},this.checked)"><input class="${inputCls}" type="number" placeholder="0" value="${s.weight||''}" oninput="updateSet(${exIdx},${si},'weight',this.value);this.classList.remove('auto-filled')" step="0.5"><input class="${repCls}" type="number" placeholder="0" value="${s.reps||s.time||''}" oninput="updateSet(${exIdx},${si},'${isTime?'time':'reps'}',this.value)" step="1">${rpe ? `<input class="set-input" type="number" placeholder="RPE" value="${s.rpe||''}" oninput="updateSet(${exIdx},${si},'rpe',this.value)" min="1" max="10" step="0.5">` : ''}</div><div class="set-delete-btn" onclick="removeSet(${exIdx},${si})">Delete</div></div><div class="pr-target-line" id="pr-line-${exIdx}-${si}"></div>`;
  }).join('');
}
...
function renderSessionExBlock(ex,idx){
  ...
  const isTime=exData&&(exData.tracking==='time'||exData.tracking==='weight_time');
  const setsHTML=(ex.sets||[]).map((s,si)=>`<div style="display:grid;grid-template-columns:28px 1fr 1fr 1fr 1fr 34px;gap:5px;align-items:center;margin-bottom:6px"><div style="font-family:var(--font-display);font-size:14px;font-weight:700;color:var(--text3);text-align:center">${si+1}</div><input class="set-input" type="number" placeholder="Wt" value="${s.weight||''}" oninput="updateSessSet(${idx},${si},'weight',this.value)" step="0.5"><input class="set-input" type="text" placeholder="${isTime?'Time':'Reps'}" value="${s.reps||s.time||''}" oninput="updateSessSet(${idx},${si},'${isTime?'time':'reps'}',this.value)"><select class="set-input" style="font-size:12px;padding:9px 4px" onchange="updateSessSet(${idx},${si},'progression',this.value)"><option value="" ${!s.progression?'selected':''}>--</option><option value="rpe" ${s.progression==='rpe'?'selected':''}>RPE</option><option value="pct1rm" ${s.progression==='pct1rm'?'selected':''}>&percnt;1RM</option><option value="rep_goal" ${s.progression==='rep_goal'?'selected':''}>Rep Goal</option></select><input class="set-input" type="text" placeholder="${s.progression==='rpe'?'RPE':s.progression==='pct1rm'?'e.g. 75':s.progression==='rep_goal'?'Reps':''}" value="${s.progressionValue||''}" oninput="updateSessSet(${idx},${si},'progressionValue',this.value)" ${!s.progression?'disabled style="opacity:0.35"':''}><button class="btn-icon" style="padding:5px;color:var(--red);border-color:rgba(255,71,87,0.4)" onclick="removeSessSet(${idx},${si})">x</button></div>`).join('');
  return`<div style="background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);padding:12px;margin-bottom:10px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><div style="font-family:var(--font-display);font-size:17px;font-weight:800;text-transform:uppercase">${name}</div><div style="display:flex;gap:5px"><button class="btn-icon" onclick="moveSessEx(${idx},-1)">&uarr;</button><button class="btn-icon" onclick="moveSessEx(${idx},1)">&darr;</button><button class="btn-icon" style="color:var(--red)" onclick="removeSessEx(${idx})">x</button></div></div><div style="display:grid;grid-template-columns:28px 1fr 1fr 1fr 1fr 34px;gap:5px;margin-bottom:5px"><div class="set-label">SET</div><div class="set-label">WT</div><div class="set-label">${isTime?'TIME':'REPS'}</div><div class="set-label">PROG</div><div class="set-label">VALUE</div><div></div></div>${setsHTML}<div style="margin-top:6px;font-size:11px;color:var(--text3)">Rep Goal / % 1RM = automated progression</div><button class="btn btn-ghost btn-sm" style="width:100%;margin-top:6px" onclick="addSessSet(${idx})">+ Set</button></div>`;
}
