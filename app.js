...
function buildRecentExpandedHtml(wid) {
  ...
  const exercisesHtml = w.exercises.map((e, ei) => {
    ...
    const videoIcon = hasVideos
      ? `<span onclick="openExpandedVideosModal('${wid}',${ei});event.stopPropagation()" style="cursor:pointer;display:inline-flex;align-items:center">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--text2)" stroke-width="2.5"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
        </span>`
      : '';
...
function editHistoricalWorkout(wid) {
  const w = appDb.workouts.find(x => x.id === wid);
  if (!w) return;
  // Load historical workout into logger — save will overwrite original
