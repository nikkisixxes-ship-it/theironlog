...
function onSetChecked(exIdx, si, checked) {
  ...
  // Run PR check and update badges immediately on complete
  if (checked) {
    if (s.weight && s.reps) {
    ...
    if (badgesEl) {
      ...
      // If badges appeared, expand the grid column
      const rowEl = document.getElementById(`set-row-${exIdx}-${si}`);
      if (rowEl && html) {
        const hasBadgeCol = rowEl.style.gridTemplateColumns.includes('auto');
        if (!hasBadgeCol) {
...
function buildSetsHTML(ex, exIdx, isTime, rpe) {
  ...
  return ex.sets.map((s, si) => {
    ...
    const gridCols = rpe
      ? `40px 1fr 1fr 64px${badgesHtml ? ' auto' : ''}`
      : `40px 1fr 1fr${badgesHtml ? ' auto' : ''}`;
