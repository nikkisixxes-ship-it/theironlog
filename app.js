...
function buildRecentExpandedHtml(wid) {
  ...
  const exercisesHtml = w.exercises.map(e => {
    ...
    const videoIcon = perfVideos.length
      ? `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--accent3)" stroke-width="2.5" style="flex-shrink:0"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>`
      : '';

    const setsHtml = e.sets.map(s => {
      let setStr = '';
      if (s.weight && +s.weight > 0) {
        setStr = `${s.weight} × ${s.reps || s.time || '—'}`;
      } else if (s.reps) {
        setStr = `BW × ${s.reps}`;
      } else if (s.time) {
        setStr = `${s.time}s`;
      ...
      return `<div class="recent-set-row">
        <span class="recent-set-str">${setStr}</span>
        ${prBadge}${e1rmBadge}
      </div>`;
    }).join('');
