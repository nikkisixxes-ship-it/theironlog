  ...
  const exercisesHtml = w.exercises.map((e, ei) => {
    ...
    const videoIcon = hasVideos
      ? `<span onclick="openExpandedVideosModal('${wid}',${ei});event.stopPropagation()" style="cursor:pointer;display:inline-flex;align-items:center">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="var(--accent3)" stroke-width="2.5"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
        </span>`
      : '';
    ...
    const setsHtml = e.sets.map(s => {
      ...
      const prBadge = s.isPR
        ? `<span class="recent-badge recent-badge-pr" style="font-size:10px;padding:1px 6px">PR: ${s.weight}</span>`
        : '';
      const e1rmBadge = s.isNew1RM && s.weight && s.reps
        ? `<span class="recent-badge recent-badge-1rm" style="font-size:10px;padding:1px 6px">E1RM: ${Math.round(calc1RM(+s.weight, +s.reps))}</span>`
        : '';
      ...
      return `<div class="recent-set-row">
        ...
        ${prBadge || e1rmBadge ? `<span class="recent-set-badges">${prBadge}${e1rmBadge}</span>` : ''}
      </div>`;
    }).join('');
