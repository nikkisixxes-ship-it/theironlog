...
function renderHomeCarousel() {
  ...
  carousel.innerHTML = homeCarouselProgs.map((prog, i) => {
    ...
    return `<div class="prog-carousel-card" onclick="${tapAction}">
      ...
      <div class="prog-card-progress-row">
        <div class="prog-card-progress-label">Progress</div>
        <div class="prog-card-progress-pct">${pct}%</div>
      </div>
  ...
  // Dots
  if (homeCarouselProgs.length > 1) {
    dotsEl.innerHTML = homeCarouselProgs.map((_, i) =>
      `<div class="home-carousel-dot ${i === homeCarouselIdx ? 'active' : ''}" onclick="goToCarouselSlide(${i})"></div>`
    ).join('');
  } else {
    dotsEl.innerHTML = '';
  }
...
function updateCarouselPosition(animate) {
  ...
  document.querySelectorAll('.home-carousel-dot').forEach((d, i) => {
    d.classList.toggle('active', i === homeCarouselIdx);
  });
}
