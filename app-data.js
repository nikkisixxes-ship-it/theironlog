...
// Returns the effective movementStructure for an exercise as an array.
// Accepts legacy string values ('Compound' / 'Isolation') transparently.
function getExMovementStructure(ex) {
  if (!ex) return [];
  let raw = ex.movementStructure || [];
  if (!ex.custom && appDb.exOverrides && appDb.exOverrides[ex.id]) {
    const ov = appDb.exOverrides[ex.id];
    if (ov.movementStructure !== undefined) raw = ov.movementStructure;
  }
  // Normalise: string → single-item array, array passthrough, empty → []
  if (typeof raw === 'string') return raw ? [raw] : [];
  return Array.isArray(raw) ? raw : [];
}

// Returns the effective executionStyle for an exercise as an array.
// Accepts legacy string values ('Bilateral' / 'Unilateral') transparently.
function getExExecutionStyle(ex) {
  if (!ex) return [];
  let raw = ex.executionStyle || [];
  if (!ex.custom && appDb.exOverrides && appDb.exOverrides[ex.id]) {
    const ov = appDb.exOverrides[ex.id];
    if (ov.executionStyle !== undefined) raw = ov.executionStyle;
  }
  if (typeof raw === 'string') return raw ? [raw] : [];
  return Array.isArray(raw) ? raw : [];
}
...
function confirm2(title, msg, cb, acceptLabel='Delete', danger=true, onReject, showCancel=true) {
  document.getElementById('confirm-title').textContent = title;
  document.getElementById('confirm-msg').textContent = msg;
  const acceptBtn = document.getElementById('confirm-accept-btn');
  acceptBtn.textContent = acceptLabel;
  acceptBtn.className = danger ? 'btn btn-danger' : 'btn btn-primary';
  // Reset on every invocation -- an OK-only informational dialog must never
  // leave Cancel hidden for a later, ordinary confirmation.
  const cancelBtn = document.querySelector('#confirm-overlay .btns .btn-secondary');
  if (cancelBtn) cancelBtn.style.display = showCancel ? '' : 'none';
  confirmCallback = cb;
  confirmRejectCallback = onReject || null;
  document.getElementById('confirm-overlay').classList.add('open');
}

function confirmAccept() {
  const acceptCb = confirmCallback;
...
function confirmReject() {
  const rejectCb = confirmRejectCallback;
  confirmCallback = null;
  confirmRejectCallback = null;
  document.getElementById('confirm-overlay').classList.remove('open');
