...
function planFormatSetSummary(s) {
  s = s || {};
  let main = '';
  if (s.timeType === 'fixed' && s.seconds) {
    main = `${s.seconds} sec`;
  } else if (s.timeType === 'range' && (s.minSeconds || s.maxSeconds)) {
    main = `${s.minSeconds || '?'}-${s.maxSeconds || '?'} sec`;
  } else {
    let loadStr = '\u2014';
...
function planEditorRow(label, sub, openAction, deleteAction, idx, total, moveUpAction, moveDownAction) {
  return `<div class="plan-row">
    <div style="flex:1;cursor:pointer" onclick="${openAction}">
      <div class="microcycle-label">${escapeHtml(label)}</div>
...
function renderPlanLibrary() {
  ...
  const progs = [...appDb.programs].sort((a, b) => {
...
function planOpenProfile(programId) {
  ...
  const structureHtml = mcs.length ? mcs.map((mc, mi) => `
    <div class="plan-row" style="cursor:pointer" onclick="planToggleProfileMc(${mi})">
      <div style="flex:1">
        <div class="microcycle-label">${escapeHtml(mc.name || `Micro ${mi + 1}`)}</div>
        <div style="font-size:13px;color:var(--text3);margin-top:2px">${(mc.sessions || []).length} session${(mc.sessions || []).length !== 1 ? 's' : ''}</div>
      </div>
    </div>
    <div id="plan-profile-mc-body-${mi}" style="display:none;padding:0 0 8px 4px"></div>
  `).join('') : `<div style="color:var(--text3);font-size:14px;padding:8px 0">No microcycles added yet</div>`;
  ...
  c.innerHTML = `
    ...
    <div style="padding:0 16px 16px">
      ${p.description ? `<div style="font-size:15px;color:var(--text2);margin-bottom:10px;line-height:1.5">${escapeHtml(p.description)}</div>` : ''}
      ${meta ? `<div class="plan-card-meta" style="margin-bottom:12px">${meta}</div>` : ''}
      ${p.notes ? `<div style="margin-bottom:16px"><div class="prof-section-label">Program Notes</div><div style="font-size:14px;color:var(--text2);line-height:1.6">${escapeHtml(p.notes).replace(/\n/g, '<br>')}</div></div>` : ''}
      ...
      <div class="prof-section-label" style="margin-bottom:8px">Program Structure</div>
      ${structureHtml}
    </div>
  `;
}
...
function planToggleProfileMc(mi) {
  ...
  body.innerHTML = sessions.length ? sessions.map(sess => {
    ...
    const exHtml = exs.length ? exs.map(ex => {
      ...
      const sets = (ex.sets || []).map(s => `<div style="font-size:13px;color:var(--text3);padding-left:8px">${planFormatSetSummary(s)}</div>`).join('');
      return `<div style="margin:6px 0 6px 12px">
        <div style="font-size:14px;font-weight:700;color:var(--text2)">${exData ? escapeHtml(exData.name) : ex.exerciseId}</div>
        ${sets || `<div style="font-size:13px;color:var(--text3);padding-left:8px">No sets added yet</div>`}
      </div>`;
    }).join('') : `<div style="font-size:13px;color:var(--text3);padding-left:12px">No exercises added yet</div>`;
    return `<div style="margin-bottom:8px">
      <div style="font-size:15px;font-weight:700;color:var(--text2);padding-left:4px">${escapeHtml(sess.name || 'Session')}</div>
      ${exHtml}
    </div>`;
  }).join('') : `<div style="color:var(--text3);font-size:13px;padding-left:8px">No sessions added yet</div>`;
...
function planStartOnTrain(programId) {
  ...
  showToast('Train integration for new templates is coming in a later stage', '');
}
...
function planRenderDetailsEditor() {
  ...
  c.innerHTML = `
    ...
    <div style="padding:16px">
      ...
      <div class="prof-section-label" style="margin:20px 0 8px">Microcycles</div>
      ${mcList}
...
function planOpenMicrocycle(mi) {
  ...
  const sessList = sessions.length ? sessions.map((s, si) => planEditorRow(
    ...
    `planMoveSession(${mi},${si},1)`
  )).join('') : `<div style="color:var(--text3);font-size:14px;padding:8px 0">No sessions added yet</div>`;
  ...
  c.innerHTML = `
    ...
    <div style="padding:16px">
      ...
      <button class="btn btn-danger" style="width:100%;margin-top:20px" onclick="planDeleteMicrocycle(${mi})">Delete This Microcycle</button>
    </div>
  `;
}
...
function planOpenSession(mi, si) {
  ...
  }).join('') : `<div style="color:var(--text3);font-size:14px;padding:8px 0">No exercises added yet</div>`;
  ...
  c.innerHTML = `
    ...
    <div style="padding:16px">
      ...
      <button class="btn btn-danger" style="width:100%;margin-top:20px" onclick="planDeleteSession(${mi},${si})">Delete This Session</button>
    </div>
  `;
}
...
function planOpenExercise(mi, si, ei) {
  ...
  const setList = sets.length ? sets.map((s, seti) => `
    <div class="plan-set-row">
      <div style="flex:1;cursor:pointer" onclick="planOpenSet(${mi},${si},${ei},${seti})">
        <span style="color:var(--text3);font-size:12px;margin-right:8px">Set ${seti + 1}</span>
        <span style="font-family:var(--font-display);font-weight:700">${planFormatSetSummary(s)}</span>
      </div>
      <div style="display:flex;gap:4px;flex-shrink:0">
  ...
  `).join('') : `<div style="color:var(--text3);font-size:14px;padding:8px 0">No sets added yet</div>`;
  ...
  c.innerHTML = `
    ...
    <div style="padding:16px">
      ...
      <button class="btn btn-danger" style="width:100%;margin-top:20px" onclick="planDeleteExercise(${mi},${si},${ei})">Remove This Exercise</button>
    </div>
  `;
}
...
function planChangeExercise(mi, si, ei) {
  openExBrowser('select', exId => {
    planDraft.microcycles[mi].sessions[si].exercises[ei].exerciseId = exId;
    planOpenExercise(mi, si, ei);
  }, 'plan');
}
...
function planOpenSet(mi, si, ei, seti) {
  ...
  c.innerHTML = `
    ...
    <div style="padding:16px">
      <div id="plan-set-preview" style="background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);padding:12px;margin-bottom:16px;text-align:center;font-family:var(--font-display);font-size:18px;font-weight:800">${planFormatSetSummary(s)}</div>

      <div class="prof-section-label">Load</div>
      <div class="form-group"><select onchange="${path}.loadType=this.value;planOpenSet(${mi},${si},${ei},${seti})">
      ...
      <div class="prof-section-label" style="margin-top:16px">Reps</div>
      <div class="form-group"><select onchange="${path}.repsType=this.value;planOpenSet(${mi},${si},${ei},${seti})">
      ...
      <div class="prof-section-label" style="margin-top:16px">Effort (RIR)</div>
      <div class="form-group"><select onchange="${path}.effortType=this.value;planOpenSet(${mi},${si},${ei},${seti})">
      ...
      <div class="prof-section-label" style="margin-top:16px">Time</div>
      <div class="form-group"><select onchange="${path}.timeType=this.value;planOpenSet(${mi},${si},${ei},${seti})">
      ...
      <div class="prof-section-label" style="margin-top:16px">Notes</div>
      <div class="form-group"><textarea placeholder="AMRAP, tempo, rest-pause, conditional instructions..." oninput="${path}.notes=this.value">${escapeHtml(s.notes || '')}</textarea></div>
    </div>
  `;
}
