...
function planFormatSetSummary(s, includeNoteTag) {
...
// Fixed values prefill the logger fields. %TM prefills the weight field too, using
// the active instance's stored Training Max (if one exists) — everything else
// (ranges, %1RM, bodyweight, RIR, notes) is preserved as read-only target context
// only, no resolution or progression logic applied here.
// exerciseId/active are optional — omitting them just skips %TM resolution.
function planSetToLoggerSet(planSet, exerciseId, active) {
  const s = planSet || {};
  ...
  out.targetSummary = hasTarget ? planFormatSetSummary(s, false) : '';
  out.targetData = { ...s };
  return out;
}
...
function planRenderProgressionSection(mi, si, ei) {
  ...
  return `
    <div class="plan-field-group">
      ...
      <div style="font-size:11px;color:var(--text3);margin-top:6px;font-style:italic;line-height:1.5">Load increases will apply to fixed-load sets only.</div>
    </div>
  `;
}
