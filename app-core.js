...
function buildProgressionReviewRow(r, idx, pending, active) {
  ...
  if (r.decision) {
    ...
    const tagClass = isSkip ? 'prog-review-done-tag prog-review-done-tag--skip' : 'prog-review-done-tag';
    const tagText = isSkip ? 'Skipped' : `Applied: +${r.appliedIncrease} ${unit}`;
...
function applyLoadIncreaseToFutureSessions(active, exerciseId, increase) {
  ...
  futureSessions.forEach(({ session }) => {
    ...
    (session.exercises || []).forEach(ex => {
      ...
      (ex.sets || []).forEach(s => {
        if (s.loadType === 'fixed' && s.weight !== '' && s.weight != null && !isNaN(parseFloat(s.weight))) {
