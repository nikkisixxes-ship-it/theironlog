...
// through this one function. It applies only when leaving an editable Collection
// route (isEditableLibraryRoute/isCollectionDraftDirty, both from app-collections.js).
// It never touches PLAN's own (separately unguarded) editor.
function showPage(id) {
  if (isEditableLibraryRoute(libraryRoute)) {
    if (isCollectionDraftDirty()) {
      confirm2(
        ...
        () => {
          ...
          showPage(id); // re-entrant call is safe: isEditableLibraryRoute(libraryRoute) is now false
        },
        'Discard'
      );
      return;
    }
    // Clean draft (or no draft) -- leave the editable route silently, no prompt needed,
    // so the user never resumes a stale/abandoned Collection screen on return.
    discardCollectionDraft();
    libraryRoute = { view: 'browser' };
  }
