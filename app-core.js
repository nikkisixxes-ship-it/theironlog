...
function ensureImportedSetsLoaded(reason) {
  ...
  importedSetsLoadPromise = COL_IMPORTED_SETS.get().then(snap => {
    if (mySession !== initSession) {
      // Signed-in user changed while this load was in flight; discard rather than
      // apply one user's data to another user's session.
  ...
  return importedSetsLoadPromise;
}
...
async function runImport(){
  ...
  const originalLabel = importBtn ? importBtn.textContent : null;
  ...
  if (existing.length>0){
    confirm2('Existing Import Detected',`You already have ${existing.length} imported sets. Importing again will add duplicates. Continue?`,()=>doImport(fileInput.files[0]),'Import Anyway');
    return;
  }
...
async function runExport(){
  let importedSets;
  try {
    importedSets = await ensureImportedSetsLoaded('export');
  } catch (err) {
    showToast('Could not load imported sets. Check your connection and try again.', 'error');
    return;
  }
