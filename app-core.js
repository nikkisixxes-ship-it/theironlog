...
function startNewWorkoutFromActive(activeId, mi, si) {
  ...
  const proceedAfterTM = () => {
    ...
    if (missing1RM.length) {
      showActive1RMPrompt(activeId, missing1RM, () => finalizeStartActiveWorkout(activeId, mi, si));
    } else {
...
function evaluateWorkoutProgression(workout, activeProgram) {
  ...
  if (!session) return [];
  const results = [];
  // V1 safeguard: only one actionable Training Max increase per exerciseId per
  // workout, even if the same exercise appears more than once in the session.
...
function buildProgressionReviewRow(r, idx, pending, active) {
  ...
  if (r.status === 'passed' && canSuggest) {
    ...
    const tmLine = isTM ? `<div class="prog-review-detail" style="margin-top:4px">Training Max: ${r.currentTM} \u2192 ${r.suggestedTM} ${unit}</div>` : '';
    detailHtml = `<div class="prog-review-detail">${targetLine}</div>${tmLine}`;
  } else {
  ...
  if (r.decision) {
    ...
    const noMatch = isTM ? (!isSkip && r.appliedTM == null) : (!isSkip && (!r.affectedSetCount || r.affectedSetCount === 0));
    ...
    if (active.status === 'complete') {
      ...
      const suggestionLine = isTM
        ? `Suggested Training Max: ${r.suggestedTM} ${unit}`
        : `Suggested increase: +${r.loadIncrease} ${unit}`;
      controlsHtml = `
...
// Edit-confirm (customValue = the user's entered number). For addLoad, customValue
// is an increment; for increaseTM, customValue is the new Training Max itself.
