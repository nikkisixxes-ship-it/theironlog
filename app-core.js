...
function getActivePrograms() {
  return appDb.programs.filter(p => {
    if (p.planSchema) return false;
    if (!Array.isArray(p.microcycles)) return false;
    ...
    return totalSessions > 0 && p.completedSessionIndices.length < totalSessions;
  });
}
...
function renderHomeCarousel() {
  homeCarouselProgs = getActivePrograms().filter(prog => {
    const next = getNextSessionForProgram(prog);
    if (!next) return false;
    const mc = prog.microcycles[next.microcycleIdx];
    return !!(mc && Array.isArray(mc.sessions) && mc.sessions[next.sessionIdx]);
  });
...
function finishWorkout() {
  ...
  appDb.workouts.unshift(currentWorkout); fsSaveWorkout(currentWorkout); stopWorkoutTimer(); currentWorkout = null; showToast('Workout saved!','success'); showPage('page-home');
}
