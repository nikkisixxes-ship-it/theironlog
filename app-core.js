...
auth.onAuthStateChanged(async user => {
  ...
  try {
    const [wSnap, pSnap, eSnap, mSnap, nSnap, iSnap, vSnap, ovSnap, collSnap] = await Promise.all([
      COL_WORKOUTS.get(), COL_PROGRAMS.get(), COL_CUSTOM_EX.get(),
      COL_META.doc('prefs').get(), COL_EX_NOTES.get(), COL_IMPORTED_SETS.get(),
      COL_EX_VIDEOS.get(), COL_EX_OVERRIDES.get(), COL_EX_COLLECTIONS.get()
    ]);
    ...
    // Firestore document ID is the identity check; persisted data still carries a
    // matching `id` field (Section 8), normalized defensively in case it's ever absent.
    appDb.exerciseCollections = collSnap.docs.map(d => {
      const data = d.data();
...
function setupListeners() {
  ...
  firestoreListeners.push(COL_CUSTOM_EX.onSnapshot(snap => {
    ...
    if (activePage() === 'page-db' && (libraryRoute.view === 'browser' || libraryRoute.view === 'collectionDetail')) {
      renderLibraryRoute();
    }
  }, () => {}));
  firestoreListeners.push(COL_EX_COLLECTIONS.onSnapshot(snap => {
    ...
    // libCollectionDrill and the modal-open state (Section 9).
