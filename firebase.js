...
const fsPlanPersistence = (function () {
  ...
  // ---- Strict input contract for fsPlanListTemplateSummaries (Phase A
  // canonical Program library correction, requirement 1). Mirrors
  // validateFsPlanReadCanonicalTemplateInput above exactly (same referential
  // Object.prototype check, same symbol/accessor/exact-own-property-set
  // rejection) -- the only difference is the accepted key set: exactly
  // { ownerUid } or { ownerUid, cursor }. cursor's own internal shape
  // (listGeneration / afterFreshnessCheckpoint / afterTemplateId) is
  // validated by listTemplateSummariesPage itself
  // (buildCanonicalPlanReaderSuite) -- never re-validated here, so this
  // wrapper reimplements none of that method's query/classification/
  // pagination/validation logic, only the minimal wrapper-boundary shape.
  function validateFsPlanListTemplateSummariesInput(input) {
    if (input === null || typeof input !== 'object') {
      throw new CanonicalPlanReaderIntegrityError('fsPlanListTemplateSummaries: input must be a plain object with ownerUid and an optional cursor');
    }
    if (Object.getPrototypeOf(input) !== Object.prototype) {
      throw new CanonicalPlanReaderIntegrityError('fsPlanListTemplateSummaries: input must be an ordinary plain object (not an array, class instance, or prototype-less object)');
    }
    if (Object.getOwnPropertySymbols(input).length !== 0) {
      throw new CanonicalPlanReaderIntegrityError('fsPlanListTemplateSummaries: input must not carry any symbol-keyed properties');
    }
    var ownNames = Object.getOwnPropertyNames(input).slice().sort();
    var hasCursorKey = ownNames.indexOf('cursor') !== -1;
    var expected = hasCursorKey ? ['cursor', 'ownerUid'] : ['ownerUid'];
    if (ownNames.length !== expected.length || ownNames.join(',') !== expected.join(',')) {
      throw new CanonicalPlanReaderIntegrityError('fsPlanListTemplateSummaries: input must have exactly the own property ownerUid, and may optionally include cursor -- got [' + ownNames.join(', ') + ']');
    }
    expected.forEach(function (key) {
      var desc = Object.getOwnPropertyDescriptor(input, key);
      if (!desc || !Object.prototype.hasOwnProperty.call(desc, 'value') || typeof desc.get === 'function' || typeof desc.set === 'function') {
        throw new CanonicalPlanReaderIntegrityError('fsPlanListTemplateSummaries: input.' + key + ' must be an ordinary data property, not an accessor');
      }
    });
    if (typeof input.ownerUid !== 'string' || !input.ownerUid) {
      throw new CanonicalPlanReaderIntegrityError('fsPlanListTemplateSummaries: input.ownerUid is required and must be a non-empty string');
    }
    if (hasCursorKey && input.cursor !== null && (typeof input.cursor !== 'object' || Array.isArray(input.cursor))) {
      throw new CanonicalPlanReaderIntegrityError('fsPlanListTemplateSummaries: input.cursor must be null or a plain object');
    }
  }

  // ---- dispatchCanonicalTemplateList(enabled, buildDeps, input) ----
  // Mirrors dispatchCanonicalTemplateRead exactly (same disabled-first
  // check, same never-inspect-input-while-disabled guarantee). The enabled
  // branch validates only the minimal wrapper-level shape above and then
  // delegates entirely to the already-accepted, already-tested reader
  // suite's own listTemplateSummariesPage method -- this function recreates
  // none of that method's query, classification,
  // pagination, or validation logic; it is a pure pass-through boundary.
  async function dispatchCanonicalTemplateList(enabled, buildDeps, input) {
    if (!enabled) {
      throw new CanonicalPlanReaderDisabledError();
    }
    validateFsPlanListTemplateSummariesInput(input);
    var suite = buildCanonicalPlanReaderSuite(buildDeps());
    return suite.listTemplateSummariesPage({ ownerUid: input.ownerUid, cursor: (input.cursor === undefined ? null : input.cursor) });
  }

  // ---- fsPlanListTemplateSummaries(input) ----
  // Phase A canonical Program library correction, requirement 1: the ninth
  // public key. Exposes the private reader suite's already-implemented,
  // already-tested listTemplateSummariesPage (exact page size 50,
  // freshness-descending + Template-Id-ascending ordering, generation-bound
  // cursors, owner checks, malformed/terminated entry classification,
  // active-item filtering) to the browser -- through the exact same
  // capability gate, deps-builder, and disabled/authorization/integrity
  // error typing fsPlanReadCanonicalTemplate above already uses. Adds no
  // new collection, no full-Template-graph read, no unrestricted reader-
  // suite exposure, and no arbitrary-owner input: ownerUid must still equal
  // the authenticated caller (enforced inside listTemplateSummariesPage
  // itself, via the reader suite's own existing owner check -- the exact
  // same guarantee fsPlanReadCanonicalTemplate already relies on for its
  // own ownerUid argument).
  async function fsPlanListTemplateSummaries(input) {
    return dispatchCanonicalTemplateList(CANONICAL_PLAN_READ_CAPABILITY_ENABLED, buildProductionCanonicalReaderDeps, input);
  }
  ...
  // correction §1; extended again by the Phase A canonical Program library
  // correction). This is the COMPLETE object in a real browser: `module`
  ...
  // these NINE keys, no more -- seven from Slice 2-4, fsPlanReadCanonicalTemplate
  // added by Slice 5A's round-2 correction, and fsPlanListTemplateSummaries
  // added by Phase A requirement 1 (the deliberately bounded wrapper around
  // the already-accepted, already-tested listTemplateSummariesPage, so the
  // browser can finally discover a signed-in owner's saved canonical
  // Programs without already knowing a Template Id).
  var publicSurface = {
    ...
    fsPlanListTemplateSummaries: fsPlanListTemplateSummaries,
  ...
  if (typeof module !== 'undefined' && module.exports) {
    publicSurface.__test = Object.freeze({
      ...
      // ---- Phase A canonical Program library correction, requirement 1.
      // Same guarded, Node-only pattern as the two seams immediately above:
      // calls the SAME dispatchCanonicalTemplateList the real, always-flag-
      // derived fsPlanListTemplateSummaries calls, with an explicitly
      // injected `enabled` boolean and injected (fake-Firestore-backed)
      // deps -- never touching, reading, or overriding
      // CANONICAL_PLAN_READ_CAPABILITY_ENABLED itself.
      dispatchCanonicalTemplateListForTest: function (enabled, deps, input) {
        return dispatchCanonicalTemplateList(enabled, function () { return deps; }, input);
      },
      validateFsPlanListTemplateSummariesInput: validateFsPlanListTemplateSummariesInput
    });
  }
...
const fsPlanListTemplateSummaries = function (input) { return fsPlanPersistence.fsPlanListTemplateSummaries(input); };
